...
async function onRequestPost(context) {
  ...
  if (ipHash) {
    try {
      const r = await db.prepare("SELECT COUNT(*) AS c FROM signups WHERE ip_hash = ?").bind(ipHash).first();
      if (Number(r?.c ?? 0) >= 25) return json({ ok: true, count: await count(db) });
    } catch {
    }
  }
  try {
    await db.prepare("INSERT INTO signups (word, contact, created_at, ip_hash) VALUES (?, ?, ?, ?)").bind(word, contact || null, Date.now(), ipHash || null).run();
  } catch {
  }
...
// ../.wrangler/tmp/pages-btPBhh/functionsRoutes-0.13357706817885373.mjs
var routes = [
  {
