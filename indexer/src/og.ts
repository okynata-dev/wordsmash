...
/** Clean monochrome OG image. 1200x630: orbit mark, word large, tagline. */
export function ogSvg(rawWord: string): string {
  ...
  const fontSize = Math.max(64, Math.min(200, Math.floor(1100 / Math.max(ticker.length, 1)) * 1.6));
  const display = escapeXml(`$${ticker}`);

  // The brand orbit in mono white: orb + ring + word-satellite at the ring's
  // upper-right — the same mark as the site header, flattened to one color.
  ...
  <circle cx="600" cy="150" r="48" fill="none" stroke="#ffffff" stroke-width="5"/>
  <circle cx="637" cy="119" r="9" fill="#ffffff"/>
  <text x="600" y="372" fill="#ffffff" font-family="ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial, sans-serif" font-size="${fontSize}" font-weight="800" text-anchor="middle" letter-spacing="-2">${display}</text>
  <text x="600" y="490" fill="#888888" font-family="ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial, sans-serif" font-size="30" letter-spacing="2" text-anchor="middle">keep a word · keepney</text>
</svg>`;
}
